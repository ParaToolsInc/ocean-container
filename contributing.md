...
Once you've obtained the Ocean docker image, here is how you use it to launch a conatiner.

You must use the flags shown here when launching the container to take advantage of certain Rift features.
...
For an example modification, here we downgrade the rubygem-text version used in Ocean's master branch from v1.3.1 to v1.3.0.

#### Create a new branch

First, create a branch to store the changes.
...
The overall diff for these changes looks like this:
```
...
The `git diff` shows that following edits are made:
...
Note where it says "Merge request pipeline #651 running." Click the pipeline number to view your merge-request's CI pipeline.
